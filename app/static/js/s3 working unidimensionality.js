...
let isUnidim = false; // treat Step 3 as unidimensional workflow when true
// Unidimensional aspects config persisted in data_step_3
let unidimAspects = []; // [{id, name, definition}]
let cviThreshold = 4; // ratings >= threshold count as agreement for I-CVI
let cvrAspectId = null; // which aspect represents Essentialness (for CVR)
let cvrThresholdOverride = null; // if set (number), use this instead of Lawshe critical
...
async function init(){
    ...
    const declaredDim = step1Data?.panel4?.dimensionality;
    isUnidim = (!declaredDim || String(declaredDim).toLowerCase() === 'unidimensional');
    if (isUnidim) {
        // Load or seed unidimensional aspects
        unidimAspects = Array.isArray(step3Data?.unidimAspects) ? step3Data.unidimAspects : [];
        if (!unidimAspects.length) {
            unidimAspects = seedDefaultUnidimAspects();
        }
        cviThreshold = (Number.isFinite(step3Data?.cviThreshold) ? step3Data.cviThreshold : 4);
        cvrAspectId = step3Data?.cvrAspectId || (unidimAspects.find(a => /essential/i.test(a.name))?.id || unidimAspects[0]?.id || null);
        cvrThresholdOverride = (step3Data?.cvrThresholdOverride !== undefined && step3Data?.cvrThresholdOverride !== null)
            ? Number(step3Data.cvrThresholdOverride)
            : null;
        // Use aspects as pseudo subdimensions for rating table
        subdimensions = unidimAspects;
    } else {
...
async function saveStep3Data() {
    ...
    if (isUnidim) {
        merged.unidimAspects = unidimAspects;
        merged.cviThreshold = cviThreshold;
        merged.cvrAspectId = cvrAspectId;
        merged.cvrThresholdOverride = (cvrThresholdOverride !== null && cvrThresholdOverride !== undefined) ? Number(cvrThresholdOverride) : null;
    }
...
// Local HTML escaper for safe UI injection in this module
...
// ============ Unidimensional CVI/CVR workflow ============
let cviTable = null;

function seedDefaultUnidimAspects() {
    // Stable IDs to avoid breaking ratings mapping between reloads
    const mk = (id, name, definition) => ({ id, name, definition });
    return [
        mk('aspect_representativeness', 'Representativeness', 'How well the item reflects the construct.'),
        mk('aspect_clarity', 'Clarity', 'The wording is clear and unambiguous.'),
        mk('aspect_essentialness', 'Essentialness', 'The item is essential for capturing the construct.')
    ];
}

function wireUnidimAnalysisUI() {
    const container = document.querySelector('#itemPanel .card-body');
    if (!container) return;
    if (document.getElementById('cvi-panel')) return; // already wired
    const panel = document.createElement('div');
    panel.id = 'cvi-panel';
    panel.className = 'mt-4';
    panel.innerHTML = `
        <div class="d-flex align-items-center justify-content-between mb-2">
            <h5 class="mb-0">Content Validity Indices (Unidimensional)</h5>
            <div class="d-flex align-items-center">
                <button type="button" id="manageAspectsBtn" class="btn btn-sm btn-outline-secondary me-2">
                    <i class="bi bi-sliders"></i> Aspects
                </button>
                <button type="button" id="runCviBtn" class="btn btn-sm btn-primary me-2">
                    <i class="bi bi-calculator"></i> Compute
                </button>
                <button type="button" id="exportCviBtn" class="btn btn-sm btn-outline-secondary me-2" disabled>
                    <i class="bi bi-download"></i> Export CSV
                </button>
                <button type="button" id="applyCviBtn" class="btn btn-sm btn-success" disabled>
                    <i class="bi bi-check2-circle"></i> Apply to Step 4
                </button>
            </div>
        </div>
        <div class="small text-secondary mb-2">I-CVI uses threshold ≥ ${cviThreshold}; CVR uses aspect “${escapeHtml(unidimAspects.find(a=>a.id===cvrAspectId)?.name || 'Essentialness')}”.</div>
        <div id="cvi-summary" class="small text-secondary mb-2"></div>
        <div id="cvi-results-table"></div>
    `;
    container.appendChild(panel);
    document.getElementById('manageAspectsBtn').onclick = showAspectsModal;
    document.getElementById('runCviBtn').onclick = async () => {
        try {
            window.showLoading && window.showLoading();
            const res = computeCviResults();
            renderCviResults(res);
            document.getElementById('exportCviBtn').disabled = res.rows.length === 0;
            document.getElementById('applyCviBtn').disabled = res.rows.filter(r=>/delete/i.test(r.action)).length === 0;
            // Persist for reload convenience
            await persistCviResults(res.rows);
            window.displayInfo && window.displayInfo('success', `Computed indices for ${res.rows.length} item(s).`);
        } catch (e) {
            console.error('CVI compute failed', e);
            window.displayInfo && window.displayInfo('danger', 'Failed to compute indices.');
        } finally {
            window.hideLoading && window.hideLoading();
        }
    };
    document.getElementById('exportCviBtn').onclick = () => exportCviCSV(lastCviResults);
    document.getElementById('applyCviBtn').onclick = applyCviToStep4;

    // Auto-load saved CVI results if present
    loadSavedCviResults();
}

function lawshesCriticalCVR(N) {
    // Lawshe (1975) critical CVR values for alpha ~.05; limited table with common N
    // Fallback to ~0.29 for N >= 40
    const table = {
        5: 0.99, 6: 0.99, 7: 0.99,
        8: 0.75, 9: 0.78, 10: 0.62, 11: 0.59, 12: 0.56, 13: 0.54, 14: 0.51,
        15: 0.49, 16: 0.47, 17: 0.45, 18: 0.43, 19: 0.42, 20: 0.42, 21: 0.40,
        22: 0.39, 23: 0.37, 24: 0.36, 25: 0.37, 26: 0.35, 27: 0.34, 28: 0.33,
        29: 0.32, 30: 0.33, 35: 0.31, 40: 0.29
    };
    if (table[N] !== undefined) return table[N];
    if (N >= 40) return 0.29;
    // Linear interpolate between nearest known keys
    const keys = Object.keys(table).map(k=>Number(k)).sort((a,b)=>a-b);
    let lo = keys[0], hi = keys[keys.length-1];
    for (const k of keys) { if (k <= N) lo = k; if (k >= N) { hi = k; break; } }
...
function computeCviResults() {
    // Snapshot current ratings
    const ratersSnapshot = raters.map(r =>
        r.id === activeRaterId ? { ...r, ratings: ratings || {} } : r
    );

    const aspectIds = unidimAspects.map(a => a.id);
    const aspectNameById = Object.fromEntries(unidimAspects.map(a => [a.id, a.name]));
    const rows = [];
    const N = ratersSnapshot.length;
    const cvrAspect = cvrAspectId && aspectIds.includes(cvrAspectId) ? cvrAspectId : null;
    const cvrCrit = (cvrThresholdOverride !== null && cvrThresholdOverride !== undefined && !Number.isNaN(Number(cvrThresholdOverride)))
        ? Number(cvrThresholdOverride) : lawshesCriticalCVR(N || 0);

    for (const it of items) {
        const perAspect = {};
        let decisionScore = 0; // simple keep/revise/delete heuristic
        for (const a of unidimAspects) {
            let nRated = 0, nAgree = 0;
            for (const r of ratersSnapshot) {
                const v = (r.ratings?.[it.id]?.[a.id]);
                const num = (v === '' || v === undefined || v === null) ? null : Number(v);
                if (Number.isFinite(num)) {
                    nRated++;
                    if (num >= cviThreshold) nAgree++;
                }
            }
            const iCVI = (nRated > 0) ? (nAgree / nRated) : null;
            perAspect[a.id] = { nRated, nAgree, iCVI };
        }
        // CVR on selected aspect
        let ne = null, cvr = null;
        if (cvrAspect && N > 0) {
            let countEssential = 0;
            for (const r of ratersSnapshot) {
                const v = (r.ratings?.[it.id]?.[cvrAspect]);
                const num = (v === '' || v === undefined || v === null) ? null : Number(v);
                if (Number.isFinite(num) && num >= cviThreshold) countEssential++;
            }
            ne = countEssential;
            cvr = (ne - (N / 2)) / (N / 2);
        }

        // Heuristic action combining Representativeness I-CVI and CVR
        const rep = perAspect['aspect_representativeness']?.iCVI ?? perAspect[aspectIds[0]]?.iCVI ?? null;
        let action = 'keep';
        if (rep !== null) {
            if (rep >= 0.78 && (cvr === null || cvr >= (cvrCrit ?? 0))) action = 'keep';
            else if (rep >= 0.66 || (cvr !== null && cvr >= (cvrCrit ?? 0) * 0.9)) action = 'revise';
            else action = 'delete';
        } else {
            action = 'revise';
        }
        ...
        rows.push({
            ...
            item_name: it.text,
            N,
            cvr_aspect: cvrAspect ? (aspectNameById[cvrAspect] || cvrAspect) : '',
            ne, cvr, cvr_crit: (N > 0 ? cvrCrit : null),
            aspects: perAspect,
            iCVI_rep: perAspect['aspect_representativeness']?.iCVI ?? null,
            iCVI_clarity: perAspect['aspect_clarity']?.iCVI ?? null,
            iCVI_ess: perAspect['aspect_essentialness']?.iCVI ?? null,
            action
        });
    }

    // S-CVI/Ave per aspect
    const sCVI = {};
    for (const a of unidimAspects) {
        const vals = rows.map(r => r.aspects?.[a.id]?.iCVI).filter(v => Number.isFinite(v));
        sCVI[a.id] = (vals.length ? (vals.reduce((s,x)=>s+x,0)/vals.length) : null);
    }
    lastCviResults = rows;
    return { rows, sCVI };
}
...
function renderCviResults(result) {
    ...
    const rows = (result?.rows || []).map(r => ({
        ...r,
        item_name: idToName.get(String(r.item)) || r.item_name || String(r.item)
    }));

    const summaryEl = document.getElementById('cvi-summary');
    if (summaryEl) {
        const keep = rows.filter(r => r.action === 'keep').length;
        const revise = rows.filter(r => r.action === 'revise').length;
        const del = rows.filter(r => /delete/i.test(r.action)).length;
        const sRep = (Number.isFinite(result?.sCVI?.['aspect_representativeness'])) ? result.sCVI['aspect_representativeness'].toFixed(3) : '-';
        summaryEl.textContent = `Keep: ${keep} • Revise: ${revise} • Delete: ${del} • S-CVI/Ave (Representativeness): ${sRep}`;
    }

    const columns = [
        { title: 'Item', field: 'item', headerSort: true, formatter: cell => {
            const id = cell.getValue();
            return idToName.get(String(id)) || String(id || '');
        }, headerTooltip: 'Item ID (cell shows text; native tooltip shows ID).', tooltip: cell => `ID: ${cell.getValue()}` },
        { title: 'N raters', field: 'N', hozAlign: 'right' },
        { title: 'I-CVI (Rep)', field: 'iCVI_rep', formatter: cell => fmtNum(cell.getValue(),3), hozAlign: 'right', headerTooltip: 'Item-level CVI for Representativeness' },
        { title: 'I-CVI (Clarity)', field: 'iCVI_clarity', formatter: cell => fmtNum(cell.getValue(),3), hozAlign: 'right' },
        { title: 'CVR aspect', field: 'cvr_aspect' },
        { title: 'n_essential', field: 'ne', hozAlign: 'right' },
        { title: 'CVR', field: 'cvr', formatter: cell => fmtNum(cell.getValue(),3), hozAlign: 'right' },
        { title: 'CVR crit', field: 'cvr_crit', formatter: cell => fmtNum(cell.getValue(),3), hozAlign: 'right' },
    ...
    if (cviTable) {
        cviTable.setColumns(columns);
        cviTable.replaceData(rows);
        if (cviTable && cviTable.options) cviTable.options.rowFormatter = rowFormatter;
        if (typeof cviTable.redraw === 'function') cviTable.redraw(true);
    } else {
        cviTable = new Tabulator('#cvi-results-table', {
            data: rows,
            columns,
            layout: 'fitDataTable',
            placeholder: 'Compute indices to see results',
            rowFormatter,
            resizableColumns: true,
            movableColumns: true,
            reactiveData: true,
...
async function persistCviResults(rows) {
    try {
        const step3 = await window.dataStorage.getData('data_step_3') || {};
        step3.cviResults = { rows: Array.isArray(rows) ? rows : [], ts: new Date().toISOString() };
        await window.dataStorage.storeData('data_step_3', step3, false);
    } catch (e) {
        console.warn('Failed to persist CVI results', e);
    }
}

async function loadSavedCviResults() {
    try {
        const step3 = await window.dataStorage.getData('data_step_3');
        const rows = Array.isArray(step3?.cviResults?.rows) ? step3.cviResults.rows : [];
        if (rows.length) {
            lastCviResults = rows;
            renderCviResults({ rows, sCVI: {} });
            const exportBtn = document.getElementById('exportCviBtn');
            const applyBtn = document.getElementById('applyCviBtn');
            if (exportBtn) exportBtn.disabled = false;
            if (applyBtn) applyBtn.disabled = rows.filter(r=>/delete/i.test(r.action)).length === 0;
        }
    } catch (e) {
        // noop
    }
...
function exportCviCSV(rows) {
    if (!Array.isArray(rows) || rows.length === 0) return;
    const idToName = new Map(items.map(it => [String(it.id), it.text]));
    const headers = ['item_name','item','N','iCVI_rep','iCVI_clarity','cvr_aspect','ne','cvr','cvr_crit','action'];
    const csv = [headers.join(',')];
    for (const r of rows) {
        const aug = { ...r, item_name: idToName.get(String(r.item)) || String(r.item) };
        const line = headers.map(h => {
            let val = aug[h];
            if (val === null || val === undefined) return '';
            if (typeof val === 'number') return String(val);
...
async function applyCviToStep4() {
    const deletable = (lastCviResults || []).filter(r => /delete/i.test(r.action)).map(r => String(r.item));
    if (deletable.length === 0) {
        window.displayInfo && window.displayInfo('info', 'No items flagged as delete.');
        return;
    }
    const ok = await (window.customConfirm ? window.customConfirm({
        title: 'Apply to Step 4?',
        message: `Exclude ${deletable.length} item(s) from the unidimensional model in Step 4?`,
        confirmText: 'Apply', cancelText: 'Cancel'
    }) : Promise.resolve(confirm('Apply deletions to Step 4?')));
    if (!ok) return;
    try {
        const s4 = await window.dataStorage.getData('data_step_4') || {};
        const cur = (s4.facetDisabledItems && Array.isArray(s4.facetDisabledItems['unidim'])) ? s4.facetDisabledItems['unidim'].map(String) : [];
        const merged = Array.from(new Set(cur.concat(deletable)));
        s4.facetDisabledItems = s4.facetDisabledItems || {};
        s4.facetDisabledItems['unidim'] = merged;
        await window.dataStorage.storeData('data_step_4', s4, false);
        window.displayInfo && window.displayInfo('success', 'Applied exclusions to Step 4.');
    } catch (e) {
        console.error('Failed to apply to Step 4', e);
        window.displayInfo && window.displayInfo('danger', 'Could not apply to Step 4.');
    }
}

// Simple aspects editor modal
function ensureAspectsModal() {
    if (document.getElementById('unidimAspectsModal')) return;
    const wrapper = document.createElement('div');
    const options = unidimAspects.map(a => `${a.name}|${a.definition || ''}`).join('\n');
    wrapper.innerHTML = `
    <div class="modal fade" id="unidimAspectsModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Unidimensional Aspects</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-2">
                        <label class="form-label small">List aspects (one per line, use “Name|Definition”)</label>
                        <textarea id="unidimAspectsInput" class="form-control" rows="6" placeholder="Representativeness|How well the item reflects the construct.\nClarity|The wording is clear and unambiguous.\nEssentialness|The item is essential for capturing the construct.">${options}</textarea>
                    </div>
                    <div class="row g-2">
                        <div class="col-6">
                            <label class="form-label small">I-CVI threshold (≥)</label>
                            <input type="number" id="cviThresholdInput" class="form-control form-control-sm" min="1" max="5" step="1" value="${Number(cviThreshold) || 4}">
                        </div>
                        <div class="col-6">
                            <label class="form-label small">CVR critical override (optional)</label>
                            <input type="number" id="cvrThresholdInput" class="form-control form-control-sm" step="0.01" value="${cvrThresholdOverride ?? ''}" placeholder="auto by N">
                        </div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label small">CVR aspect</label>
                        <select id="cvrAspectSelect" class="form-select form-select-sm"></select>
                    </div>
                    <div class="form-text small mt-2">Ratings are on 1–5. I-CVI treats ratings ≥ threshold as “agreement”. CVR treats ratings ≥ threshold as “essential”.</div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary" id="saveAspectsBtn">Save</button>
                </div>
            </div>
        </div>
    </div>`;
...
function populateCvrAspectOptions() {
    const sel = document.getElementById('cvrAspectSelect');
    if (!sel) return;
    sel.innerHTML = unidimAspects.map(a => `<option value="${a.id}" ${a.id===cvrAspectId ? 'selected' : ''}>${escapeHtml(a.name)}</option>`).join('');
}

async function showAspectsModal() {
    ensureAspectsModal();
    populateCvrAspectOptions();
    const modalEl = document.getElementById('unidimAspectsModal');
    const bsModal = new bootstrap.Modal(modalEl);
    // Update textarea with current aspects before show
    const ta = modalEl.querySelector('#unidimAspectsInput');
    ta.value = unidimAspects.map(a => `${a.name}|${a.definition || ''}`).join('\n');
    modalEl.querySelector('#cviThresholdInput').value = Number(cviThreshold) || 4;
    modalEl.querySelector('#cvrThresholdInput').value = (cvrThresholdOverride ?? '');

    modalEl.querySelector('#saveAspectsBtn').onclick = async () => {
        const lines = ta.value.split(/\n+/).map(s => s.trim()).filter(Boolean);
        const next = [];
        const nameToExisting = new Map(unidimAspects.map(a => [a.name.toLowerCase(), a]));
        for (const line of lines) {
            const [name, def] = line.split('|');
            const nm = (name || '').trim();
            const df = (def || '').trim();
            if (!nm) continue;
            const existing = nameToExisting.get(nm.toLowerCase());
            if (existing) next.push({ ...existing, name: nm, definition: df }); else {
                next.push({ id: `aspect_${crypto.randomUUID ? crypto.randomUUID() : String(Date.now()+Math.random())}`, name: nm, definition: df });
            }
        }
        if (!next.length) {
            window.displayInfo && window.displayInfo('warning', 'At least one aspect is required.');
            return;
        }
        unidimAspects = next;
        // Update selected CVR aspect
        const sel = modalEl.querySelector('#cvrAspectSelect');
        cvrAspectId = (sel && sel.value) ? sel.value : next[0].id;
        // Update thresholds
        const cviIn = modalEl.querySelector('#cviThresholdInput');
        const cvrIn = modalEl.querySelector('#cvrThresholdInput');
        const cv = Number(cviIn.value);
        cviThreshold = (Number.isFinite(cv) && cv >= 1 && cv <= 5) ? Math.round(cv) : 4;
        const cvrVal = cvrIn.value;
        cvrThresholdOverride = (cvrVal === '' || cvrVal === null || cvrVal === undefined) ? null : Number(cvrVal);
        // Persist config and refresh table columns
        subdimensions = unidimAspects;
        await saveStep3Data();
        renderRatingTable();
        // Update header note
        const note = document.querySelector('#cvi-panel .small.text-secondary');
        if (note) note.textContent = `I-CVI uses threshold ≥ ${cviThreshold}; CVR uses aspect “${escapeHtml(unidimAspects.find(a=>a.id===cvrAspectId)?.name || 'Essentialness') }”.`;
        bsModal.hide();
        window.displayInfo && window.displayInfo('success', 'Aspects saved.');
    };
    bsModal.show();
}
 ...
 * 
 * actions taken
Step 3 detects unidimensional constructs and switches to a CVI/CVR workflow.
Added unidimensional aspects config:
Defaults: Representativeness, Clarity, Essentialness.
Manage via an “Aspects” modal: edit list, I-CVI threshold (default ≥4), optional CVR critical override, and choose which aspect drives CVR.
Persists in data_step_3 (unidimAspects, cviThreshold, cvrAspectId, cvrThresholdOverride).
Rating table adapts to aspects for unidimensional case (columns become aspects).
CVI/CVR computation:
I-CVI per aspect and item: proportion of ratings ≥ threshold.
S-CVI/Ave per aspect (summary includes Representativeness).
CVR using Lawshe’s formula on the chosen aspect; includes critical CVR threshold (auto by N or overridden).
Heuristic action per item: keep / revise / delete.
Results UI:
New panel “Content Validity Indices (Unidimensional)” with Compute, Export CSV, Apply to Step 4, and Aspects buttons.
Tabulated results with row highlights by action and summary counts.
CSV export of CVI/CVR results.
Apply to Step 4: merges items flagged “delete” into Step 4’s facetDisabledItems['unidim'].
Safe UI escaping and persistence:
Added escapeHtml helper.
Saved and auto-loads last computed CVI results in data_step_3.cviResults.
files changed
step3.js
Detected unidimensional mode, added aspects config, CVI/CVR compute and render, export and apply integration, aspects modal, and local HTML escaper.
Preserved existing multi-facet ANOVA path for non-unidimensional.
try it
Go to Step 3 with a unidimensional construct (Step 1 → Dimensionality = Unidimensional).
Add raters and rate items against the aspects (columns are Representativeness, Clarity, Essentialness).
Click “Aspects” to tweak aspects, thresholds, and CVR aspect if desired.
Click “Compute” to see per-item I-CVI (Rep, Clarity) and CVR, suggested actions, and a summary.
Export results via “Export CSV”.
Click “Apply to Step 4” to exclude “delete” items from the unidimensional model (they’ll appear as excluded in Step 4).
notes
For multi-facet models, the original ANOVA-based analysis remains unchanged.
CVR critical values default by rater count (Lawshe). You can override in the Aspects modal.
All new data is stored in data_step_3 and respects current project scope.
quality gates
Lint/Type: PASS (no static errors in step3.js).
Build/Tests: not applicable here; no backend/API changes.
Requirements coverage:

Unidimensional content validity flow (UI+metrics+apply): Done.
CSV export for unidim results: Done.
Persist aspects and thresholds: Done.
 */
